...
- If continuing an ongoing objective, call `get_goal` before acting. If context appears missing after compaction, or exact earlier details matter, call `recall_history` before re-reading files or asking the user. After compaction, subagent `task_id`s are recovered and injected as a user message; reuse them directly. If a needed sidekick or reviewer `task_id` is missing but likely exists, recover it before starting a new subagent session.
