...
- If continuing an ongoing objective, call `get_goal` before acting. If context appears missing after compaction, or exact earlier details matter, call `recall_history` before re-reading files or asking the user. After compaction, subagent `task_id`s are preserved in the compaction summary's Critical Context; reuse them directly. If a needed sidekick or reviewer `task_id` is missing from the summary, call `get_task_ids` to recover them from message history before starting a new subagent session. If recovery fails, create a new subagent session rather than taking over the investigation yourself. Do not default to solo execution when delegation was the intended mode.
